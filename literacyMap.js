(function() {
    d3.queue()
        ...
        .await(function(error, topoMain, topoKashmir) {
            ...
            var filter  = 'zone' //d3.select('#select input[name="gender"]:checked').node().value;
...
function colorCode2(data, filter) {
...
function colorCode(data, filter) {
    data.forEach(function(d) {
        //if (isNaN(d.properties[filter])) { d.color = 'black'; }
        try {d.color = d.properties['zone'].toLowerCase(); } catch (err) { d.color = 'black'; }
    });
}
